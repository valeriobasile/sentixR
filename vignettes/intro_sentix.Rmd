...
`sentixr` is a package designed to simplify sentiment analysis in Italian using a variety of lexicons (Sentix, MAL, ELIta).

This vignette illustrates the core workflow using the high-level functions `sentix_annotate()` and `sentix_summarize()`, and some of the main features of `sentix_annotate()`.
...
```{r}
sentix_summarize(frase_ann, simplify = TRUE)
```
...
The `sentix_annotate()` function performs tokenization and lemmatization (via `udpipe`), and then joins the result with one of the available sentiment lexicons. By default, it uses Sentix.
...
The function, like *udpipe*, accepts as input single texts, multiple texts (a character vector, a list or a list of tokens), or data frames with `text` and `doc_id` columns.

For large corpora, the user may optionally specify the number of cores to use, via the argument `parallel.cores`, which is inherited from *udpipe* and passed to `udpipe::udipe()`.

The function processes single texts and multiple texts, assigning a document ID to each one, and helps the user specify document identifiers, which are processed and safely passed to `udpipe::udpipe()`.
...
*udpipe* accepts data frames with text and document ID columns (`text` and `doc_id`). `sentix_annotate()` also allows specifying the input column names. Note that the function extracts and processes only these two columns, ignoring other metadata.
...
-   '1' (Low Variation): Difference between the max and below the mean;
...
-   `n_poly`: Number of tokens with ambiguous tokens, based on the `ambiguity` level setting (default = 3);
-   `n_scored`: Number of tokens found in the lexicon.
