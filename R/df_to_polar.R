...
#' polarity dictionary.(see  \code{\link[quanteda]{dictionary}}).
...
df_to_polar <- function(x,
  ...
  polarity_assignment <- list()
  if (!is.null(polar_map)) {
    if (!is.character(polar_map) || is.null(names(polar_map))) {
      stop("polar_map must be a named character vector.")
    }
    polarity_assignment <- as.list(polar_map)
  } else {
    dict_keys <- names(final_list)
    lk <- tolower(dict_keys)
    if (any(lk %in% c("positive", "pos"))) {
      polarity_assignment$pos <- dict_keys[lk %in%
                                             c("positive", "pos")]
    }
    if (any(lk %in% c("negative", "neg"))) {
      polarity_assignment$neg <- dict_keys[lk %in%
                                             c("negative", "neg")]
    }
    if (any(lk %in% c("neutral", "neut"))) {
      polarity_assignment$neut <- dict_keys[lk %in%
                                              c("neutral", "neut")]
    }
  }
  if (length(polarity_assignment) > 0) {
    quanteda.sentiment::polarity(quanteda_dict) <- polarity_assignment
  } else {
    warning(
