...
sentix_summarize <- function(x,
  ...
  if (length(missing_groups) > 0) {
    stop(paste0(
      ...
      "Grouping columns not found: ", paste(shQuote(missing_groups), collapse = ", ")
    ))
  }
  ...
  if (is.null(cols)) {
    ...
    possible_cols <- c(
      ...
      "fiducia", "sorpresa", "aspettativa", "amore"
    )
    score_cols <- intersect(names(x), possible_cols)
    
    if (length(score_cols) == 0) {
      # numeric columns, excluding udpipe output
      exclude_cols <- c("doc_id", "paragraph_id", "sentence_id", "token_id", 
                        "head_token_id", "polypathy_index", "ambiguity", 
                        "start", "end", "term_id", "upos_id", "dep_rel", "misc")
  ...
  summary_df <- x |>
    ...
    dplyr::summarise(
      ...
      n_poly = if ("polypathy_index" %in% names(x) && ambiguity != "none") {
        sum(is_ambiguous(.data$polypathy_index) & .data$.has_any_score, na.rm = TRUE)
      } else {
        NA_real_
      },
